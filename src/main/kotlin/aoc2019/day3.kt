...
fun day3_1(wire1: String, wire2: String): Int {
    val steps1 = wire1.split(",")
    val steps2 = wire2.split(",")

    val field = mutableMapOf<XY, Int>()
    fill(steps1, field, 1)
    fill(steps2, field, 2)
...
fun day3_2(wire1: String, wire2: String): Int {
    val steps1 = wire1.split(",")
    val steps2 = wire2.split(",")

    val field1 = mutableMapOf<XY, Int>()
    val field2 = mutableMapOf<XY, Int>()
    fill(steps1, field1)
    fill(steps2, field2)

    return field1
        .filterKeys { field2.contains(it) }
        .map { it.value + field2.getOrDefault(it.key, 0) }
        .min() ?: -1
}

private fun fill(steps: List<String>, field: MutableMap<XY, Int>, mask: Int) {
    var x = 0
    var y = 0
    ...
    steps.forEach { step ->
        when {
            step.startsWith("U") -> {
                ...
                (1..d).forEach { i ->
                    val key = XY(x, y + i)
                    field[key] = field.getOrDefault(key, 0) or mask
                }
                y += d
            }
            step.startsWith("R") -> {
...
private fun fill(steps: List<String>, field: MutableMap<XY, Int>) {
    var x = 0
    var y = 0
    ...
    steps.forEach { step ->
        when {
            step.startsWith("U") -> {
                ...
                (1..d).forEach { i ->
                    val key = XY(x, y + i)
                    field[key] = field.getOrDefault(key, ++dist)
                }
                y += d
            }
            step.startsWith("R") -> {
